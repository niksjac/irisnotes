...
import { ChevronRight, ChevronDown, FileText, Book, FolderOpen, Folder } from "lucide-react";
  ...
  const tree = useTree<FlexibleItem>({
    ...
    dataLoader: {
      getItem: (itemId: string): FlexibleItem => {
        ...
        if (!item) {
          console.error(`Item with id ${itemId} not found. Available items:`, Array.from(itemMap.keys()));
      ...
      <div className="flex-1 overflow-auto p-2">
        <div {...tree.getContainerProps()} className="space-y-1">
          {tree.getItems().map((item) => {
            ...
            const isFolder = itemData.type === 'book' || itemData.type === 'section';
