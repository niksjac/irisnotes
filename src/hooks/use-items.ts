...
import { itemsAtom, selectedItemIdAtom, selectedItemAtom, treeDataAtom } from "@/atoms/items";
...
export const useItems = () => {
	...
	const createItem = useCallback(async (params: CreateItemParams) => {
		setError(null);
	...
	const updateItem = useCallback(async (id: string, updates: Partial<FlexibleItem>) => {
		setError(null);
		...
		let result: any;
			if (item.type === 'note') {
				...
				if (updates.content !== undefined) noteParams.content = updates.content;
				if (updates.content_type) noteParams.content_type = updates.content_type;
				if (updates.content_raw !== undefined) noteParams.content_raw = updates.content_raw;
				if (updates.parent_id !== undefined) noteParams.parent_id = updates.parent_id;
	...
	const searchItems = useCallback(async (query: string, filters?: { type?: 'note' | 'book' | 'section' }) => {
		setError(null);
	...
	const createNote = useCallback((params: Omit<CreateNoteParams, 'content_type'>) => {
		return createItem({
			type: 'note',
			title: params.title,
			content: params.content,
	...
	const createBook = useCallback((title: string, parent_id?: string) => {
		return createItem({
			type: 'book',
			title,
			parent_id,
	...
	const createSection = useCallback((title: string, parent_id?: string) => {
		return createItem({
			type: 'section',
			title,
			parent_id,
	...
	const updateItemContent = useCallback((id: string, content: string, contentType?: 'html' | 'markdown' | 'plain' | 'custom', contentRaw?: string) => {
