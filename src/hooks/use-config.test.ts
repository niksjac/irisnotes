...
describe('useConfig', () => {
  ...
  describe('initialization', () => {
    it('should initialize with default config and loading state', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('read_config', { filename: './dev/config/app-config.json' })
      expect(result.current.config.editor).toEqual({ lineWrapping: true, toolbarVisible: false })
    })
  ...
  describe('updateConfig', () => {
    ...
    it('should merge nested config properties correctly', async () => {
      ;(invoke as any)
        .mockResolvedValueOnce(JSON.stringify(DEFAULT_CONFIG))
      ...
      await act(async () => {
        await result.current.updateConfig({ editor: { lineWrapping: true, toolbarVisible: true } })
      })
